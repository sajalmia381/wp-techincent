...
function penci_posted_on() {
	...
	if ( ! penci_get_setting( 'penci_hide_single_date' ) ) {
		...
		$time_string = sprintf( $time_string, ! $show_updated ? esc_attr( get_the_date( 'c' ) ) : esc_attr( get_the_modified_date( 'c' ) ), ! $show_updated ? esc_html( get_the_date() ) : esc_html( get_the_modified_date() ), esc_attr( get_the_modified_date( 'c' ) ), esc_html( get_the_modified_date() ) );
...
function penci_posted_on_archive( $page = '' ) {
	...
	if ( ! $hide_post_author ) {
		$byline = sprintf( esc_html_x( '%s', 'post author', 'pennews' ), '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>' );
	...
	if ( ! $hide_date ) {
		...
		$posted_on = sprintf( esc_html_x( '%s', 'post date', 'pennews' ), '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>' );
...
function penci_get_post_date( $show = true ) {
	...
	$posted_on = sprintf( esc_html_x( '%s', 'post date', 'pennews' ), '<a class="penci_pmeta-link"  href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>' );
...
function penci_entry_footer() {
	...
	edit_post_link( sprintf( /* translators: %s: Name of current post */ esc_html__( 'Edit %s', 'pennews' ), the_title( '<span class="screen-reader-text">"', '"</span>', false ) ), '<span class="edit-link">', '</span>' );
}
...
function penci_get_categories() {
	...
	foreach ( $cat_list as $cat ) {
		$categories_list .= '<a href="' . get_category_link( $cat->term_id ) . '">' . $cat->name . '</a>';
	}
...
if ( ! function_exists( 'penci_get_archive_image_type' ) ) {
	function penci_get_archive_image_type( $img_size, $type_img = '' ) {
		...
		if ( 'square' == $type_img ) {
			...
			} elseif ( 'penci-thumb-480-320' == $img_size || 'penci-thumb-480-645' == $img_size || 'penci-masonry-thumb' == $img_size ) {
