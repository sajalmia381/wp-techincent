...
function penci_scripts() {
	...
	if ( $fontawesome_ver5 ) {
		wp_enqueue_style( 'penci-font-awesomev5', get_template_directory_uri() . '/css/font-awesome.5.11.2.swap.min.css', array(), '5.11.2' );
	}
	...
	if ( defined( 'PENCI_DEPLOY_MODE' ) && PENCI_DEPLOY_MODE ) {
        ...
        wp_enqueue_script( 'penci-script-lib', get_template_directory_uri() . '/js/script.lib.min.js', array(
            'fitvids',
            'velocity',
            'imagesloaded',
            'owl.carousel',
		...
		wp_enqueue_script( 'penci', get_template_directory_uri() . '/js/script.js', array('penci-script-lib'), PENCI_PENNEWS_VERSION, true );

	} else {
        wp_enqueue_script( 'penci-script-lib', get_template_directory_uri() . '/js/script.lib.min.js', array('jquery'), PENCI_PENNEWS_VERSION, true );
		wp_enqueue_script( 'penci', get_template_directory_uri() . '/js/script.min.js', array( 'jquery', 'penci-script-lib' ), PENCI_PENNEWS_VERSION, true );
	}
...
if( !function_exists('penci_admin_scripts') ) {
	function penci_admin_scripts( $hook ) {
		...
		if ( $hook == 'widgets.php' || 'nav-menus.php' == $hook ) {
			...
			wp_enqueue_media();
			wp_enqueue_script( 'penci-admin-widget', get_template_directory_uri() . '/js/admin-widget.js', array( 'jquery'), PENCI_PENNEWS_VERSION, true );
