	...
	window.VcPenciContainerView = vc.shortcode_view.extend({
		...
		buildDesignHelpers: function() {
			...
			var valueParam = '',
				...
				elDisableSticky = this.model.getParam("el_disable_sticky");
			...
			if( ! _.isEmpty( elWidth ) ) {
				...
				var $twoSidebar =  this.$el.find("> .vc_controls .cell-14_12_14"),
					$fourSidebar =  this.$el.find("> .vc_controls .cell-14_14_14_14");

				if( 'penci-wide-content' === elWidth ) {
					valueParam += '';
				}else if( 'penci-container' === elWidth && 'penci_container' === this.model.attributes.shortcode ) {
					valueParam += 'W:1080px';
				...
				}else if( 'penci-container-1170' === elWidth ) {
					valueParam += 'W:1170px';
					$twoSidebar.addClass( 'penci-hidden' );
					$fourSidebar.addClass( 'penci-hidden' );
				...
				} else if( 'penci-container-fluid' === elWidth ) {
					valueParam += 'W:1400px';
