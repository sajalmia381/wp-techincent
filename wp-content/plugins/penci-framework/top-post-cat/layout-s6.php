...
$post_placeholder  = 7 - intval( $query_cat_grid->post_count );
?>
		...
		<div class="penci-container">
			<div class="penci-div-inner">
				...
				while ( $query_cat_grid->have_posts() ) :
					...
					if ( $bgrid_count == 5 || $cat_grid_i == $query_cat_grid->post_count ) {
						if( $post_placeholder ) {
							for( $i = 1; $i <= $post_placeholder; $i ++ ) {
								$cat_grid_i2 = $cat_grid_i + $i;
								?>
								<article <?php post_class( 'penci-post-item ' . ( 4 > $cat_grid_i2 ? 'penci-pitem-big' : 'penci-pitem-small'  ) . ' penci-pitem-placeholder penci-pitem-' . $cat_grid_i2 ); ?>>
									<?php
									echo Penci_Helper_Shortcode::get_image_holder_pre( array(
										'hide_review' => true,
									) );
