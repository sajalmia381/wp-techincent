...
function penci_post_like_callback() {
	$nonce = $_POST['nonce'];
	...
	if ( isset( $_POST['penci_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( $count_meta_post != 0 ) { // meta exists, set up values
			...
			if ( $count_meta_users != 0 ) { // meta exists, set up values
			...
			if ( $count_meta_IPS != 0 ) { // meta exists, set up values
...
function penci_get_post_like_link( $post_id, $show = false, $class_pre ="" ) {
	...
	if ( penci_Already_Liked( $post_id ) ) {
