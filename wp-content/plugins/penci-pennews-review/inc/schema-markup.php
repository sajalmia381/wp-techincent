...
if ( ! class_exists( 'Penci_Review_Schema_Markup' ) ):
	class Penci_Review_Schema_Markup {
		...
		public static function amp_json_schema(){
			...
			if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'penci_review') ) {
				...
				$json = array();
				if( $schema_markup_type && 'none' != $schema_markup_type ) {
					if( 'Product' == $schema_markup_type ){
						$json = self::output_schema_product( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Restaurant' == $schema_markup_type ){
						$json = self::output_schema_restaurant( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Book' == $schema_markup_type ){
						$json = self::output_schema_book( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Game' == $schema_markup_type ){
						$json = self::output_schema_game( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Movie' == $schema_markup_type ){
						$json = self::output_schema_movie( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'MusicRecording' == $schema_markup_type ){
						$json = self::output_schema_musicrecording( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'SoftwareApplication' == $schema_markup_type ){
						$json = self::output_schema_software( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Store' == $schema_markup_type ){
						$json = self::output_schema_store( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'TVSeries' == $schema_markup_type ){
						$json = self::output_schema_store( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Course' == $schema_markup_type ){
						$json = self::output_schema_course( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}elseif( 'Event' == $schema_markup_type ){
						$json = self::output_schema_event( $penci_review, $schema_markup_type, $schema_type_val, $total_average, $review_id );
					}
			...
			$json = array(
				...
				'itemReviewed' => array(
					...
					'review' => array(
						...
						'reviewBody'   => self::get_review_des( $penci_review ),
					)
				),
				'reviewRating' => array(
					'@type'       => 'Rating',
		...
		public static function get_review_des( $penci_review ){
			return isset( $penci_review['penci_review_des'] ) ? $penci_review['penci_review_des'] : '';
		}
